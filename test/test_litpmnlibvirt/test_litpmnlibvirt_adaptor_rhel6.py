...
class TestLitpLibVirtAdaptor(unittest.TestCase):
    ...
    def setUp(self):
        self.adaptor = LitpLibVirtAdaptor("unittest", base_os="6")
    ...
    def test_force_restart_returns_value_from_start(self, fstop, start):
    ...
    @mock.patch("litpmnlibvirt.litp_libvirt_adaptor.LitpLibVirtAdaptor.start")
    @mock.patch("litpmnlibvirt.litp_libvirt_adaptor.LitpLibVirtAdaptor.stop")
    def test_restart_returns_value_from_start(self, stop, start):
        stop.return_value = 1
        start.return_value = 2
    ...
    def test_force_stop_calls_destroy_if_domain_defined_and_running(self,
            _is_def, _is_run, _get_dom, _log, lv_succ):
        destroy = mock.Mock()
    ...
    @mock.patch(ADAPTOR_CLASS + "._is_running")
    @mock.patch(ADAPTOR_CLASS + "._is_defined")
    def test_force_stop_calls_destroy_fails(self, _is_def, _is_run, _get_dom, _log):
        _is_def.return_value = True
        _is_run.return_value = True
...
class TestLitpLibVirtAdaptorHelpers(unittest.TestCase):
    ...
    def setUp(self):
        self.adaptor = LitpLibVirtAdaptor("helper_test", base_os='6')
